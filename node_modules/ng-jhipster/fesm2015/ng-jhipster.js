...
import { Injectable, Component, Input, Directive, Output, EventEmitter, Pipe, Host, HostListener, ContentChild, ElementRef, Sanitizer, SecurityContext, defineInjectable, inject, forwardRef, NgModule } from '@angular/core';
...
class JhiItemCountComponent {
    ...
    i18nValues() {
        /** @type {?} */
        const first = (this.page - 1) * this.itemsPerPage === 0
            ? 1
            : (this.page - 1) * this.itemsPerPage + 1;
        /** @type {?} */
        const second = this.page * this.itemsPerPage < this.total
...
JhiMaxValidatorDirective.decorators = [
    { type: Directive, args: [{
                ...
                providers: [
                    { provide: NG_VALIDATORS, useExisting: forwardRef(() => JhiMaxValidatorDirective), multi: true }
                ]
            },] }
];
/** @nocollapse */
...
class JhiFilterPipe {
     ...
     * @return {?}
     */
    filterByStringAndField(filter$$1, field) {
        return (value) => {
     ...
     * @return {?}
     */
    filterByObject(filter$$1) {
        return (value) => {
            /** @type {?} */
            const keys = Object.keys(filter$$1);
            for (let i = 0; i < keys.length; i++) {
                ...
                let isMatching;
                if (type === 'string') {
                    isMatching = this.filterByString(filter$$1[key])(value[key]);
                }
                else if (type === 'object') {
                    isMatching = this.filterByObject(filter$$1[key])(value[key]);
                }
                else {
                    isMatching = this.filterDefault(filter$$1[key])(value[key]);
                }
                if (!isMatching) {
                    return false;
                }
     ...
     * @return {?}
     */
    transform(input, filter$$1, field) {
        if (!filter$$1) {
            return input;
        }
        /** @type {?} */
        const type = typeof filter$$1;
        if (type === 'string') {
            if (field) {
...
class JhiPureFilterPipe extends JhiFilterPipe {
     ...
     */
    transform(input, filter$$1, field) {
        return super.transform(input, filter$$1, field);
    }
}
...
class JhiEventManager {
    ...
    subscribe(eventName, callback) {
        /** @type {?} */
        const subscriber = this.observable.pipe(filter((event) => {
